...
export default function LeadDetailPage() {
  ...
  const [editActForm,  setEditActForm]  = useState({ title: '', type: 'call', scheduled_at: '', notes: '' })
  const [editActSaving, setEditActSaving] = useState(false)
  const [editActError,  setEditActError]  = useState('')
  ...
  async function handleUpdateActivity(e: React.FormEvent) {
    ...
    try {
      if (!editActForm.title || !editActForm.scheduled_at) {
        setEditActError('Title and Date & Time are required.')
      ...
      const { data, error } = await supabase
        .from('activities')
        .update({
          title: editActForm.title,
          type: editActForm.type,
          scheduled_at: new Date(editActForm.scheduled_at).toISOString(),
          notes: editActForm.notes || null,
        })
        .eq('id', editingAct.id)
        .select()
        .single()
      ...
      if (error) {
        setEditActError('Could not save: ' + error.message + (error.code ? ' (code ' + error.code + ')' : ''))
        setEditActSaving(false)
        return
      }
      ...
      setEditActError('Unexpected error: ' + (err?.message || String(err)))
    }
  ...
  async function handleDeleteActivity() {
    ...
    if (error) {
      setEditActError('Could not delete: ' + error.message)
      setEditActSaving(false)
      return
    }
    ...
    <AppLayout title={lead.full_name}>
              ...
              <div className="space-y-2">
                {activities.map(act => (
                  <div key={act.id}
                    ...
                    className={`bg-white rounded-2xl border border-[#E2E8F4] px-4 py-3 flex items-start gap-3 cursor-pointer hover:bg-[#FAFBFE] transition-colors ${act.completed ? 'opacity-60' : ''}`}>
                    ...
                    <button onClick={(e) => { e.stopPropagation(); toggleActivity(act.id, act.completed) }}>
                      {act.completed
      ...
      {editingAct && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end md:items-center md:justify-center">
          <div className="bg-white w-full md:w-96 rounded-t-3xl md:rounded-2xl p-6 pb-8">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-[#0D1B3E]">Edit Activity</h3>
              <button onClick={closeEditActivity} className="text-[#9AAAC8]"><X size={20}/></button>
            </div>
            {editActError && (
              <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-sm text-red-600 mb-3">
                {editActError}
            ...
            <form onSubmit={handleUpdateActivity} className="space-y-3">
              <div>
                <label className="text-xs font-semibold text-[#4A5880] mb-1.5 block">Title *</label>
                <input required value={editActForm.title}
                  onChange={e => setEditActForm(p => ({ ...p, title: e.target.value }))}
              ...
              <div>
                <label className="text-xs font-semibold text-[#4A5880] mb-1.5 block">Date & Time *</label>
                <input required type="datetime-local" value={editActForm.scheduled_at}
                  onChange={e => setEditActForm(p => ({ ...p, scheduled_at: e.target.value }))}
                  className="w-full border border-[#E2E8F4] rounded-xl px-4 py-3 text-sm outline-none focus:border-[#075290] bg-[#FAFBFE]"/>
              </div>
              <div>
                <label className="text-xs font-semibold text-[#4A5880] mb-1.5 block">Notes</label>
                <textarea value={editActForm.notes} onChange={e => setEditActForm(p => ({ ...p, notes: e.target.value }))}
              ...
              <div className="flex gap-2 mt-2">
                <button type="button" onClick={handleDeleteActivity} disabled={editActSaving}
                ...
                <button type="submit" disabled={editActSaving}
                  ...
                  {editActSaving ? <Loader2 size={16} className="animate-spin"/> : <Check size={16}/>}
                  Save Changes
                </button>
              </div>
            </form>
